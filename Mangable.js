...
var Mangable = {
     ...
     $.ajax(
        {
          ...
          success: function( objResponse ){
            ...
            $("#comprehensive_list li .col1 > a:first-child", div).each(function(index) {
              var tit = $(this).text();
              res[res.length] = [tit, "http://mangable.com" + $(this).attr("href")];
            });
  ...
  getListImages : function(doc, curUrl) {
    ...
    $("#select_page select option", doc).each(
      function(index){
        res[index] = currentChapterURL + $(this).val() + "/";
      }
    );
    return res;
  },
  ...
  removeBanners : function(doc, curUrl) {
    //This function runs in the DOM of the current consulted page.
    //$("div > iframe", doc).each(function(index){$(this).parent().remove()});
    $("iframe", doc).remove();
    $("#set1", doc).remove();
  ...
  isCurrentPageAChapterPage : function(doc, curUrl) {
    return ($("#image_box #image", doc).length > 0);
  },
  //This method is called before displaying full chapters in the page
  doSomethingBeforeWritingScans : function(doc, curUrl) {
    //This function runs in the DOM of the current consulted page.
    $("#image_box td a", doc).remove();
    $("#image_box td > img", doc).remove();
    $("#chapter_choose_container", doc).remove();
    console.log("Sizeeee : " + $("#image_box td", doc).length);
    $("<div class='scansAMR'></div>").appendTo($("#image_box td", doc));
