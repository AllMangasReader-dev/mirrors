var Imangascans = {
    ...
    isMe : function (url) {
        "use strict";
        return (url.indexOf("imangascans.org") !== -1);
    },
    getMangaList : function (search, callback) {
        "use strict";
        $.ajax({
            url : "http://reader.imangascans.org/",
            ...
            success : function (objResponse) {
                ...
                $("select[name='manga'] option", div).each(function (index) {
                    if ($(this).attr('value') !== 0) {
                        res[res.length] = [$(this).text(), 'http://reader.imangascans.org/' + $(this).attr('value')];
                    }
                });
    ...
    getInformationsFromCurrentPage : function (doc, curUrl, callback) {
        ...
        var currentMangaURL = 'http://reader.imangascans.org/' + $("select[name='manga'] option[selected]", doc).attr('value');
        var currentChapterURL = currentMangaURL + '/' + $("select[name='chapter'] option[selected]", doc).attr('value');
        callback({
            "name" : name,
    ...
    getListImages : function (doc, curUrl) {
        ...
        var res = [],
            ...
            base = /"pg_base":(".*?")/gi,
            pages = /"pages":(\[.*?\])/gi;
        imgUrl = "http://reader.imangascans.org/" + JSON.parse(base.exec($('body').html())[1]);
        img = JSON.parse(pages.exec($('body').html())[1]);
