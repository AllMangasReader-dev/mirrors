var MangaInn = {
    ...
    languages : "en",
    isMe : function (url) {
        return (url.indexOf("mangainn.com/") !== -1);
    },
    getMangaList : function (search, callback) {
        $.ajax({
            url : "http://www.mangainn.com/advresults",
            type : 'POST',
            data : {
                "seriestype" : "c",
                "series" : search,
            ...
            success : function (objResponse) {
                ...
                $(".content .TurqHeadLabel", div).each(function (index) {
                    res[res.length] = [$(this).text().trim(), 'http://www.mangainn.com/manga/' + $(this).attr("onclick").toString().replace("function onclick(event) {", "").replace("MangaInfo", "").replace("}", "").replace("('", "").replace("')", "").trim()];
                });
    ...
    isCurrentPageAChapterPage : function (doc, curUrl) {
        return (curUrl.search('www.mangainn.com/manga/chapter') > -1);
    },
    ...
    nextChapterUrl : function (select, doc, curUrl) {
        if ($(select).children("option:selected").next().size() !== 0) {
            return "http://www.mangainn.com/manga/chapter/" + $(select).children("option:selected").next().val();
        }
        return null;
    },
    previousChapterUrl : function (select, doc, curUrl) {
        if ($(select).children("option:selected").prev().size() !== 0) {
            return "http://www.mangainn.com/manga/chapter/" + $(select).children("option:selected").prev().val();
        }
        return null;
    },
