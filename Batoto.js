var Batoto = {
    ...
    isMe : function (url) {
        "use strict";
        return (url.match(/batoto.(com|net)/gi) !== null);
    },
    getMangaList : function (search, callback) {
        "use strict";
        var res = [];
        $.ajax({
            url : 'http://www.batoto.net/search?name=' + search + '&name_cond=c',
                ...
                $("#comic_search_results .chapters_list tr a", div).each(function (index) {
                    $.ajax({
                        ...
                        success : function (objResponse) {
                            ...
                            divIn.innerHTML = objResponse.replace(/<img/gi, '<noload');
                            if ($(".lang_English:eq(-1) a:first", divIn) !== null) {
                                res[res.length] = [$(".ipsType_pagetitle", divIn).text().trim(), $(".lang_English:eq(-1) a:first", divIn).attr('href')];
                            }
                            console.log(res)
   ...
   getListImages : function (doc, curUrl) {
    ...
    isCurrentPageAChapterPage : function (doc, curUrl) {
        "use strict";
        return ($("#comic_page", doc).size() > 0 || $("#full_image", doc).size() > 0 || curUrl.indexOf(/read/) !== -1);
    },
